...
class ReclinerGraphUI extends CustomPainter {
  ...
  ReclinerGraphUI(
      {required this.healthItem, required this.data, required this.labels});
  ...
  void drawBar(Canvas canvas, Size size, List<Offset> coordinates) {
    ...
    if(data.length > 7){
      xAxisUnit = size.width / (data.length + 1);
    }else{
    ...
    for (int idx = 0; idx < data.length; idx++) {
      ...
      double yMove = size.height - ((size.height - 2*bottomPadding)*data[idx])/rangeMax-bottomPadding;
  ...
  @override
  bool shouldRepaint(ReclinerGraphUI old) {
